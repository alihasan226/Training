...
import com.oasissnacks.oasissnacks.databinding.FragmentDisclaimerBinding;
import com.oasissnacks.oasissnacks.utils.AppUser;
import com.oasissnacks.oasissnacks.utils.LocalRepositories;

public class DisclamierFragment  extends Fragment {
    public FragmentDisclaimerBinding binding;
    AppUser appUser=new AppUser();
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding= DataBindingUtil.inflate(inflater, R.layout.fragment_disclaimer,container,false);
        appUser= LocalRepositories.getAppUser(getActivity().getApplicationContext());
        if(appUser!=null&&!appUser.disclamir.equalsIgnoreCase(""))
        {
            binding.webViewdisclaimer.loadData(appUser.disclamir,"text/html","UTf-8");
        }
